...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('eval_qwen_results.log'),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger('eval_qwen')

# 환경 초기화 과정에서 자세한 로그 추가
try:
    logger.info("환경 초기화 시작")
    config = Config()
    logger.info("Config 초기화 성공")
    
    model_clients = ModelClients(config)
    logger.info("ModelClients 초기화 성공")
    
    video_processor = VideoProcessor()
    logger.info("VideoProcessor 초기화 성공")
    
    tip_calculator = TipCalculator(config, model_clients, video_processor)
    logger.info("TipCalculator 초기화 성공")

    # Qwen 클라이언트 초기화 확인
    if model_clients.qwen_client is None:
        logger.error("Qwen 클라이언트 초기화 실패")
        sys.exit(1)
    
    logger.info(f"Qwen API 키 확인: {config.QWEN_API_KEY[:5]}******")
    logger.info("Qwen 클라이언트 초기화 및 연결 성공")
...
def generate_qwen_prediction(text_content, situation_caption):
    """Qwen을 사용하여 Reddit 게시물에서 팁 예측 생성"""
    
    logger.info("Qwen 예측 생성 시작")
    
    # 프롬프트 설정 (config에서 DEFAULT_PROMPT_TEMPLATE 사용)
    prompt = f'''
###Persona###
...
'''
    ...
    try:
        logger.info("Qwen API 호출 시작")
        
        # Qwen 모델 호출
        response = model_clients.qwen_client.chat.completions.create(
        ...
        if not result:
            logger.warning(f"숫자 응답을 찾을 수 없음: '{response_text[:200]}...'")
            # 텍스트는 저장하기 위해 None 대신 튜플 형태로 반환 (예측값 None, 프롬프트, 응답)
            return None, prompt, response_text
        
        logger.info(f"추출된 숫자 문자열: '{result}'")
        
        try:
            # 소수점이 마지막에 위치한 경우 제거 (예: "15.")
            if result.endswith('.'):
                result = result[:-1]
                logger.info(f"소수점 제거 후: '{result}'")
                
            # 소수점이 처음에 위치한 경우 제거 (예: ".5")
            if result.startswith('.'):
                result = '0' + result
                logger.info(f"소수점 처음 위치 처리 후: '{result}'")
            ...
            logger.info(f"팁 퍼센트 변환 성공: {tip_percentage}%")
            
            # 범위 검증
            if 0 <= tip_percentage <= 100:
                logger.info(f"유효한 팁 퍼센트: {tip_percentage}%")
                return tip_percentage, prompt, response_text
            else:
        ...
        logger.error(f"Qwen 예측 중 오류 발생: {e} - 이 컬럼은 pass합니다.")
...
def evaluate_model(max_samples=10):
    ...
    for i, item in enumerate(dataset):
        ...
        logger.info(f"상황 설명 길이: {len(situation_caption)} 문자")
        
        # Qwen 예측 생성
        try:
            result = generate_qwen_prediction(text_content, situation_caption)
            ...
            if predicted_tip is None:
                ...
                results.append({
                    'situation_caption': situation_caption,
    ...
    if predicted_tips:  # 성공적으로 예측된 데이터가 있는 경우
        ...
        # CSV 파일로 결과 저장 (프롬프트와 응답 제외, 성공한 레코드만)
        try:
            csv_file = 'eval_qwen_results.csv'
            with open(csv_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=['situation_caption', 'text_content', 'actual_tip', 'predicted_tip', 'error', 'status'])
                writer.writeheader()
        ...
        try:
            json_file = 'eval_qwen_results_full.json'
        ...
        try:
            ...
            with open('eval_qwen_stats.json', 'w', encoding='utf-8') as f:
                json.dump(stats, f, indent=2)
            
            logger.info("평가 통계가 eval_qwen_stats.json에 저장되었습니다.")
        except Exception as e:
...
if __name__ == "__main__":
    logger.info("Qwen 모델을 사용한 Reddit 게시물 기반 팁 예측 평가 시작")
    evaluate_model(max_samples=105)  # 3개 샘플만 처리 (디버깅용)
    logger.info("평가 완료") 