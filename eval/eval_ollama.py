...
logger = logging.getLogger('eval_ollama')

# Ollama 클라이언트 설정
try:
    client = Client(host='http://localhost:11434')
    client.list()  # Ollama 서버 연결 테스트
    logger.info("Ollama 클라이언트 초기화 및 연결 성공")
except Exception as e:
    logger.error(f"Ollama 클라이언트 초기화 또는 연결 실패: {e}")
    sys.exit(1)
...
def generate_ollama_prediction(text_content, situation_caption, model_name="mistral-small3.1:latest"):
    """Ollama를 사용하여 Reddit 게시물에서 팁 예측 생성"""
...
'''
    ...
    try:
        response = client.generate(
            ...
            options={
                "temperature": 0.1,
                "num_gpu": 99,  # llm_server.py와 동일하게 GPU 코어 수 설정
                "top_p": 0.95,
            }
        )
        ...
        logger.error(f"Ollama 예측 중 오류 발생: {e} - 이 컬럼은 pass합니다.")
...
def evaluate_model(max_samples=10):
    ...
    for i, item in enumerate(dataset):
        ...
        # Ollama 예측 생성
        try:
            result = generate_ollama_prediction(text_content, situation_caption)
    ...
    if predicted_tips:  # 성공적으로 예측된 데이터가 있는 경우
        ...
        try:
            json_file = 'eval_results_full.json'
        ...
        try:
            ...
            with open('eval_stats.json', 'w', encoding='utf-8') as f:
                json.dump(stats, f, indent=2)
            
            logger.info("평가 통계가 eval_stats.json에 저장되었습니다.")
        except Exception as e:
...
if __name__ == "__main__":
    logger.info("Ollama 모델을 사용한 Reddit 게시물 기반 팁 예측 평가 시작")
    evaluate_model(max_samples=105)  # 10개 샘플만 처리 # 112
    logger.info("평가 완료") 